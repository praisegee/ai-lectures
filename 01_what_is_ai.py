...
def main():
    ...
    paradigms = [
        {"category": "Supervised", "signal": "labeled examples", "example": "spam detection"},
        {"category": "Unsupervised", "signal": "raw data only", "example": "customer clustering"},
        {"category": "Reinforcement", "signal": "rewards & penalties", "example": "game playing"},
    ]  # @inspect paradigms
    ...
    pipeline = ["collect data", "preprocess", "define model", "train", "evaluate", "deploy"]  # @inspect pipeline
    ...
    plot({
        ...
        "width": 400,
        "height": 250,
        "data": {"values": [{"hours": h, "score": s} for h, s in zip(hours, scores)]},
        "mark": {"type": "point", "size": 80, "filled": True, "color": "#4f8ef7"},
        "encoding": {
            "x": {"field": "hours", "type": "quantitative", "title": "Hours studied"},
            "y": {"field": "score", "type": "quantitative", "title": "Exam score", "scale": {"zero": False}},
        },
    })
